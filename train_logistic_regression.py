...
from sklearn.linear_model import LogisticRegression  # 导入sklearn逻辑回归分类器
from sklearn.model_selection import StratifiedKFold  # 导入sklearn分层K折交叉验证
from sklearn.preprocessing import StandardScaler  # 导入sklearn标准化预处理
...
def cross_validation(X, y, n_splits=5):
    """
    执行5折分层交叉验证，评估Logistic Regression模型性能
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):  # 遍历每一折
        ...
        scaler = StandardScaler()  # 创建标准化器，用于特征标准化
        X_train_scaled = scaler.fit_transform(X_train)  # 对训练集进行标准化（拟合+转换）
        X_test_scaled = scaler.transform(X_test)  # 对测试集进行标准化（仅转换）
        
        model = LogisticRegression(  # 创建逻辑回归分类器实例
            C=1.0,  # 正则化强度的倒数，值越小正则化越强
            max_iter=1000,  # 最大迭代次数为1000，确保算法收敛
            solver='lbfgs',  # 使用L-BFGS优化算法，适用于中小规模数据
            random_state=42  # 随机种子为42，确保结果可复现
        )
        
        model.fit(X_train_scaled, y_train)  # 使用标准化后的训练数据拟合模型
        
        y_pred = model.predict(X_test_scaled)  # 对标准化后的测试集进行预测
        y_prob = model.predict_proba(X_test_scaled)[:, 1]  # 获取正类预测概率
    ...
    print(f"\n{'='*50}")  # 打印分隔线
    print("Cross-Validation Results (Logistic Regression):")  # 打印交叉验证结果标题
    print(f"{'='*50}")  # 打印分隔线
    ...
    results = {  # 创建结果字典
        'model': 'Logistic Regression',  # 模型名称
...
if __name__ == '__main__':  # 主程序入口
    ...
    print("="*60)  # 打印分隔线
    print("Logistic Regression Model Training")  # 打印模型训练标题
    print("="*60)  # 打印分隔线
    ...
    with open('results/lr_results.json', 'w') as f:  # 打开结果文件
        json.dump(results, f, indent=2)  # 保存结果到JSON文件
    
    print(f"\nResults saved to results/lr_results.json")  # 打印结果保存提示
