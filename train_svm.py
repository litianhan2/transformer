...
from sklearn.svm import SVC  # 导入sklearn支持向量机分类器
...
def cross_validation(X, y, n_splits=5):
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):  # 遍历每一折
        ...
        model = SVC(  # 创建支持向量机分类器实例
            C=1.0,  # 正则化参数，控制对误分类的惩罚程度
            kernel='rbf',  # 使用径向基函数（高斯核）作为核函数
            gamma='scale',  # 核系数设置为'scale'，自动计算为1/(n_features * X.var())
            probability=True,  # 启用概率估计，允许计算预测概率
    ...
    print(f"\n{'='*50}")  # 打印分隔线
    print("Cross-Validation Results (SVM):")  # 打印交叉验证结果标题
    print(f"{'='*50}")  # 打印分隔线
...
if __name__ == '__main__':  # 主程序入口
    ...
    print("="*60)  # 打印分隔线
    print("SVM Model Training")  # 打印模型训练标题
    print("="*60)  # 打印分隔线
    ...
    with open('results/svm_results.json', 'w') as f:  # 打开结果文件
        json.dump(results, f, indent=2)  # 保存结果到JSON文件
    
    print(f"\nResults saved to results/svm_results.json")  # 打印结果保存提示
