...
from sklearn.ensemble import RandomForestClassifier  # 导入sklearn随机森林分类器
...
def cross_validation(X, y, n_splits=5):
    """
    执行5折分层交叉验证，评估Random Forest模型性能
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):  # 遍历每一折
        ...
        model = RandomForestClassifier(  # 创建随机森林分类器实例
            n_estimators=200,  # 决策树数量为200
            max_depth=20,  # 决策树最大深度为20
    ...
    print(f"\n{'='*50}")  # 打印分隔线
    print("Cross-Validation Results (Random Forest):")  # 打印交叉验证结果标题
    print(f"{'='*50}")  # 打印分隔线
...
if __name__ == '__main__':  # 主程序入口
    ...
    print("="*60)  # 打印分隔线
    print("Random Forest Model Training")  # 打印模型训练标题
    print("="*60)  # 打印分隔线
    ...
    with open('results/rf_results.json', 'w') as f:  # 打开结果文件
        json.dump(results, f, indent=2)  # 保存结果到JSON文件
    
    print(f"\nResults saved to results/rf_results.json")  # 打印结果保存提示
